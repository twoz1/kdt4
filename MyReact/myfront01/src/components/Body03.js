...
//   const onChangeBirth = (e) =>{setName(e.target.value)};
//   const onChangeInfo = (e) =>{setName(e.target.value)};
...
// => 코드가 간결해지고 효율적
// => 수정사항 
//   - Stage 객체 생성 , 이벤트 핸들러 1개로 동일
//   - 엘리먼트들의 value 속성값, name 속성 추가, 이벤트 핸들러 
...
const onChangeState =(e)=>{
    console.log('수정대상 :  '+ e.target.name); //이벤트가 일어난 대상의 
...
//   const onChangeInfo = (e) =>{setName(e.target.value)};
  ...
  return (
    ...
    <div>
        <input type="date" name='name' value={state.birth} onChange={onChangeState} />
    </div>
