...
public class Lm02_doubleColon {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < colors.size(); i++) {
			System.out.println("color2 :" +colors.get(i) ); //get은 메서드임 배열이 아니기에[]사용 노노
		}

		// 2. forEach 메서드를 이용한 Lamda 적용
		// 매개변수를 Cousumer형태의 클래스와 String or Object만 올 수 있음
		// => void forEach(Consumer<? super String> action)
		// => Consumer<? super String> : 같거나 상위타입 가능(String or Object만 올 수 있음)
		// => Consumer 클래스 소스 확인   
		//     - @FunctionalInterface
		//    - void accept(T t);   
		...
		// 결과적으로 메서드를 매개변수 받는 식이 완성됨.
		// 메서드를 매개변수로 독립적으로 받을 수 없기에 자바는 함수형언어가 아니다라고 불렸음.
		// 그러나 람다식이 만들어지면서 매개변수로 받을 수 있기에 함수형 언어라고 불림
		// 자바스크립트에서는 이것이 너무 당연했었음.
	...
	// String 은 종단 클래스 아무것도 상속받지 않음 -> 그냥 Object의 후손
}//class
