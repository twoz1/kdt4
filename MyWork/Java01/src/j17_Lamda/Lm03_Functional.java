...
interface Value{
	int num(int a , int b);
	//String myString(int a , int b);  //int를 두개 받아서 String으로 ..
}

class Compute{
	// Value를 매개변수로 가져다 쓰는 메서드
	public void inValue(Value v) {
		int result = v.num(5, 10);
		// => num 메서드의 처리내용은 구현이 안되어 있음
		//    invalue 메서드 호출시 넣어주는 인자(Value 인터페이스)에의해 결정됨.
...
public class Lm03_Functional {
	...
	public static void main(String[] args) {
		...
		// 3. 생략형 람다식 (-연산으로 구현)
		System.out.println("** 2. 생략형 람다식 (- 연산으로 구현)**");
		cp.inValue((a,b)-> a - b);
	}
