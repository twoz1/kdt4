...
@Override
	public void replyInsert() {System.out.println("** replyInsert **");}
}
...
class NoticeBoard extends BoardA {
	...
	@Override
	public void listPrint() {System.out.println("**  NoticeBoard listPrint **");}
	@Override
...
public class Ex06_AbsInter {
	...
	public static void main(String[] args) {
		...
		//interface 보다 적은 메서드 구현
		//interface의 모든 메서드를 오버라이딩한 추상클래스를 상속 받았음
		bi.update();
		bi.listPrint();
		
		BoardA ba = new QnABoard();
		// => BoardA를 상속 받은 NoticeBoard()와는 교체 가능
		// => BoardA를 상속 받은 않은  MyBoard로는 생성할 수 없음
		ba.listPrint();
		ba.delete();
		// => QnABoard에는 정의하지 않았지만 ,조상인 BoardA에 있으므로 가능함
		// => 그러나 BoardA 에는 없고,  QnABoard 에만 정의한 메서드가 있다면 사용 불가능.
	}
